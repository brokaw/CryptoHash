...
class CryptoHashTests: XCTestCase {
    ...
    func testMD5DigestUpdateFunctionEqualsOneShotFunction() {
        var context = MD5_Init()
        MD5_Update(&context, "H".data(using: .utf8)!)
        MD5_Update(&context, "e".data(using: .utf8)!)
        MD5_Update(&context, "l".data(using: .utf8)!)
        MD5_Update(&context, "l".data(using: .utf8)!)
        MD5_Update(&context, "o".data(using: .utf8)!)
        MD5_Update(&context, " ".data(using: .utf8)!)
        MD5_Update(&context, "W".data(using: .utf8)!)
        MD5_Update(&context, "o".data(using: .utf8)!)
        MD5_Update(&context, "r".data(using: .utf8)!)
        MD5_Update(&context, "l".data(using: .utf8)!)
        MD5_Update(&context, "d".data(using: .utf8)!)
        let digest = MD5_Final(context)
        let expected = Data([0xb1, 0x0a, 0x8d, 0xb1, 0x64, 0xe0, 0x75, 0x41,
                             0x05, 0xb7, 0xa9, 0x9b, 0xe7, 0x2e, 0x3f, 0xe5])
        XCTAssertEqual(digest, expected)
    }
    func testSHA1DigestUpdateFunctionEqualsOneShotFunction() {
    ...
    func testSHA224DigestUpdateFunctionEqualsOneShotFunction() {
        var context = SHA224_Init()
        SHA224_Update(&context, "H".data(using: .utf8)!)
        SHA224_Update(&context, "e".data(using: .utf8)!)
        SHA224_Update(&context, "l".data(using: .utf8)!)
        SHA224_Update(&context, "l".data(using: .utf8)!)
        SHA224_Update(&context, "o".data(using: .utf8)!)
        SHA224_Update(&context, " ".data(using: .utf8)!)
        SHA224_Update(&context, "W".data(using: .utf8)!)
        SHA224_Update(&context, "o".data(using: .utf8)!)
        SHA224_Update(&context, "r".data(using: .utf8)!)
        SHA224_Update(&context, "l".data(using: .utf8)!)
        SHA224_Update(&context, "d".data(using: .utf8)!)
        let digest1 = SHA224_Final(context)
        let digest2 = self.data.SHA224Digest()
    ...
    func testSHA256DigestUpdateFunctionEqualsOneShotFunction() {
        ...
        SHA256_Update(&context, "l".data(using: .utf8)!)
        SHA256_Update(&context, "l".data(using: .utf8)!)
        SHA256_Update(&context, "o".data(using: .utf8)!)
        SHA256_Update(&context, " ".data(using: .utf8)!)
        SHA256_Update(&context, "W".data(using: .utf8)!)
        SHA256_Update(&context, "o".data(using: .utf8)!)
        SHA256_Update(&context, "r".data(using: .utf8)!)
        SHA256_Update(&context, "l".data(using: .utf8)!)
        SHA256_Update(&context, "d".data(using: .utf8)!)
        let digest1 = SHA256_Final(context)
        let digest2 = self.data.SHA256Digest()
        XCTAssertEqual(digest1, digest2)
    }
    func testSHA384DigestUpdateFunctionEqualsOneShotFunction() {
        var context = SHA384_Init()
        SHA384_Update(&context, "H".data(using: .utf8)!)
        SHA384_Update(&context, "e".data(using: .utf8)!)
        SHA384_Update(&context, "l".data(using: .utf8)!)
        SHA384_Update(&context, "l".data(using: .utf8)!)
        SHA384_Update(&context, "o".data(using: .utf8)!)
        SHA384_Update(&context, " ".data(using: .utf8)!)
        SHA384_Update(&context, "W".data(using: .utf8)!)
        SHA384_Update(&context, "o".data(using: .utf8)!)
        SHA384_Update(&context, "r".data(using: .utf8)!)
        SHA384_Update(&context, "l".data(using: .utf8)!)
        SHA384_Update(&context, "d".data(using: .utf8)!)
        let digest1 = SHA384_Final(context)
        let digest2 = self.data.SHA384Digest()
        XCTAssertEqual(digest1, digest2)
    }
    func testSHA512DigestUpdateFunctionEqualsOneShotFunction() {
        var context = SHA512_Init()
        SHA512_Update(&context, "H".data(using: .utf8)!)
        SHA512_Update(&context, "e".data(using: .utf8)!)
        SHA512_Update(&context, "l".data(using: .utf8)!)
        SHA512_Update(&context, "l".data(using: .utf8)!)
        SHA512_Update(&context, "o".data(using: .utf8)!)
        SHA512_Update(&context, " ".data(using: .utf8)!)
        SHA512_Update(&context, "W".data(using: .utf8)!)
        SHA512_Update(&context, "o".data(using: .utf8)!)
        SHA512_Update(&context, "r".data(using: .utf8)!)
        SHA512_Update(&context, "l".data(using: .utf8)!)
        SHA512_Update(&context, "d".data(using: .utf8)!)
        let digest1 = SHA512_Final(context)
        let digest2 = self.data.SHA512Digest()
        XCTAssertEqual(digest1, digest2)
    }
}
